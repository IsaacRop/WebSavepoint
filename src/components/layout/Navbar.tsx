...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-paper-50 border-b border-ink-10">
      <div className="max-w-[1360px] mx-auto px-6 h-14 flex items-center gap-8">
        {/* Logo */}
        <Link
          href="/feed"
          className="font-sans font-medium text-ink text-base tracking-tight flex-shrink-0 flex items-center gap-2"
        >
          <FlagIcon />
          savepoint
        </Link>
            ...
            return (
              <Link
                ...
                className={[
                  ...
                  active
                    ? "text-ink border-b border-terra pb-[1px]"
        ...
        <div className="flex items-center gap-4">
          ...
          {searchOpen ? (
            ...
            <button
              ...
              className="text-ink-50 hover:text-ink transition-colors duration-[120ms]"
              aria-label="Buscar"
            >
              <SearchIcon />
            </button>
          ...
          {/* Hamburger — mobile */}
          <button
            className="md:hidden text-ink-50 hover:text-ink transition-colors"
      ...
      {/* Mobile drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-paper-50 border-t border-ink-10 px-6 py-4 flex flex-col gap-4">
          {NAV_LINKS.map(({ label, href }) => (
            <Link
              key={href}
              href={href}
